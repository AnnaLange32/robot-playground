...
def ordered_grid_maker():
    ...
    bipolar_grid = np.zeros((constants.width, constants.length, constants.rsize[0]*constants.rsize[0]))
    image_v = []
    image_number = 0
    for j in range(constants.width):
        ...
        for i in range(constants.length):
            ...
            filename = constants.get_gameimages + '%s' % image_number + '.png'
            image_number += 1
            bipolar_image = bipolarize_pattern_robot(filename, constants.rsize)
            image = cv2.imread(filename, cv2.IMREAD_COLOR)
            img = cv2.resize(image, constants.rsize)
            white = [255, 255, 255]
            img = cv2.copyMakeBorder(img, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=white)
            image_h.append(img)
            bipolar_grid[j, i, :] = bipolar_image
        concat_h = cv2.hconcat(image_h)
        image_v.append(concat_h)
    image_grid_final = cv2.vconcat(image_v)
    print('size grid:', image_grid_final.size)
    filename = constants.store_grids + 'basegrid'
...
def make_yellow_frames(yellow_number):
    ...
    for j in range(constants.width):
        ...
        for i in range(constants.length):
            ...
            filename = constants.get_gameimages + '%s' % image_number + '.png'
            image = cv2.imread(filename, cv2.IMREAD_COLOR)
            rimg = cv2.resize(image, constnats.rsize)
    ...
    image_grid_final = cv2.vconcat(image_v)
    filename = constants.store_base + 'yellowgrid%s.png' % yellow_number
    cv2.imwrite(filename, image_grid_final)
